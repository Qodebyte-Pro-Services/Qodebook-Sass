...
exports.incomeExpenseOverTime = async (req, res) => {
  try {
    ...
    if (business_id) {
      orderWheres.push(`o.business_id = $${idx}`);
      orderParams.push(business_id);
      idx++;
    }
    if (branch_id) {
      orderWheres.push(`o.branch_id = $${idx}`);
      orderParams.push(branch_id);
      idx++;
    }
    if (start_date && end_date) {
      orderWheres.push(`o.created_at::date BETWEEN $${idx} AND $${idx + 1}`);
    ...
    const incomeResult = await pool.query(
      `SELECT ${orderDateSelect} AS period,
    ...
    if (business_id) {
      expenseWheres.push(`e.business_id = $${expIdx}`);
      expenseParams.push(business_id);
      expIdx++;
    }
    if (start_date && end_date) {
      expenseWheres.push(`e.created_at::date BETWEEN $${expIdx} AND $${expIdx + 1}`);
    ...
    const expenseResult = await pool.query(
      `SELECT ${expenseDateSelect} AS period,
...
exports.getBudgetAnalytics = async (req, res) => {
  try {
    ...
    const {business_id, period, start_date, end_date } = req.query;
    ...
    let dateFilter = "";
    let values = [business_id];

    switch (period) {
      case "today":
        dateFilter = `AND TO_DATE(period_start, 'YYYY-MM-DD') = CURRENT_DATE`;
        break;

      case "yesterday":
        dateFilter = `AND TO_DATE(period_start, 'YYYY-MM-DD') = CURRENT_DATE - INTERVAL '1 day'`;
        break;

      case "this_week":
        dateFilter = `AND TO_DATE(period_start, 'YYYY-MM-DD') >= date_trunc('week', CURRENT_DATE)`;
        break;

      case "month":
        dateFilter = `AND budget_month = TO_CHAR(NOW(), 'Month') 
                      AND budget_year = TO_CHAR(NOW(), 'YYYY')`;
        break;

      case "year":
        dateFilter = `AND budget_year = TO_CHAR(NOW(), 'YYYY')`;
        break;
      ...
      case "custom":
        ...
        dateFilter = `AND TO_DATE(period_start, 'YYYY-MM-DD') BETWEEN $2 AND $3`;
    ...
    const query = `
      SELECT 
        COALESCE(SUM(amount) FILTER (WHERE status = 'approved'), 0) AS total_budget,
        COALESCE(SUM(budget_spent), 0) AS total_budget_spent,
        COALESCE(SUM(budget_remaining), 0) AS total_remaining,
        COUNT(*) FILTER (WHERE status = 'pending') AS total_pending_budget,
        COUNT(*) FILTER (WHERE status = 'approved') AS total_approved_budget,
        COUNT(*) FILTER (WHERE status = 'rejected') AS total_rejected_budget
      FROM budgets
      WHERE business_id = $1
      ${dateFilter}
    `;
    ...
    const highestCategoryQuery = `
      SELECT ec.name AS category_name, b.amount
      FROM budgets b
      JOIN expense_categories ec ON b.category_id = ec.id
      WHERE b.business_id = $1
        AND b.status = 'approved'
        ${dateFilter.replace(/\$1/g, '$1')} 
      ORDER BY b.amount DESC
      LIMIT 1
    `;

    
    const lowestCategoryQuery = `
      SELECT ec.name AS category_name, b.amount
      FROM budgets b
      JOIN expense_categories ec ON b.category_id = ec.id
      WHERE b.business_id = $1
    ...
    const [analyticsResult, highestCatResult, lowestCatResult] = await Promise.all([
      pool.query(query, values),
      pool.query(highestCategoryQuery, values),
      pool.query(lowestCategoryQuery, values)
    ]);
    ...
    return res.status(200).json({
      ...
      analytics: {
        ...
        total_budget_spent: Number(data.total_budget_spent),
        total_remaining: Number(data.total_remaining),
        total_pending_budget: Number(data.total_pending_budget),
        total_approved_budget: Number(data.total_approved_budget),
        total_rejected_budget: Number(data.total_rejected_budget),
        highest_budget_category: highestCatResult.rows[0] || null,
        lowest_budget_category: lowestCatResult.rows[0] || null
      },
    });
...
exports.overview = async (req, res) => {
  try {
    ...
    const expenseDateWhere = buildDateWhere(date_filter, start_date, end_date, "e.");
    ...
    const countWhere = buildWhere({ business_id, branch_id }, "");
    ...
    const [
      ...
      budgetByCategoryResult,
      orderStatsResult,
      cogsResult,
      salesRevenueResult,
      topProductsResult,
      totalExpenseResult,
      staffSalaryExpenseResult,
      topExpenseCategoryResult,
      topBudgetCategoryResult,
      variantsCountResult,
      variantsInStockResult,
      staffWithShiftTodayResult,
      stockMovementResult,
      topStockMovementResult,
      serviceTrackingResult,
      productsCountResult,
      staffCountResult,
      ...
      pool.query(
        `SELECT 
          ...
          COALESCE(SUM(discount_total + coupon_total), 0) AS total_discounts,
          COALESCE(SUM(tax_total), 0) AS total_taxes
         FROM orders o
         WHERE o.status = 'completed'${orderWhere}${dateWhere}`,
        orderParams
      ),
      ...
      pool.query(
        `SELECT SUM(amount) AS total_expense
         FROM expenses e
         WHERE e.status = 'approved'${expenseWhere}${expenseDateWhere}`,
        expenseParams
      ),
      ...
      pool.query(
        `SELECT COALESCE(SUM(e.amount), 0) AS total_staff_salary_paid
         ...
         AND (LOWER(ec.name) = 'salary' OR e.staff_id IS NOT NULL)${expenseWhere}${expenseDateWhere}`,
        expenseParams
      ),
      ...
      pool.query(
        `SELECT COUNT(*) as count FROM products WHERE TRUE${countWhere.clause}`,
        countWhere.params
      ),
      pool.query(
        `SELECT COUNT(*) as count FROM staff WHERE TRUE${countWhere.clause}`,
        countWhere.params
      ),
      pool.query(
        `SELECT COUNT(*) as count FROM customers WHERE TRUE${countWhere.clause}`,
        countWhere.params
      ),
      pool.query(
        `SELECT COUNT(*) as count FROM services WHERE TRUE${countWhere.clause}`,
        countWhere.params
      ),
    ]);

   
    const grossIncome = Number(orderStatsResult.rows[0]?.gross_income || 0);
    const discounts = Number(orderStatsResult.rows[0]?.total_discounts || 0);
    const taxes = Number(orderStatsResult.rows[0]?.total_taxes || 0);
    ...
    const staffWithShiftToday = Number(staffWithShiftTodayResult.rows[0]?.count || 0);
...
exports.expenseStatsByCategory = async (req, res) => {
  try {
    ...
    const totalExpenses = parseInt(totalRes.rows[0].total_expenses, 10);
    if (totalExpenses === 0) {
    ...
    const categoryRes = await pool.query(
      `SELECT c.name AS category_name,
              ...
              ROUND((COUNT(e.id)::decimal / $2) * 100, 2) AS percentage
       FROM expenses e
       JOIN expense_categories c ON e.category_id = c.id
       WHERE e.business_id = $1
       GROUP BY c.name
       ORDER BY expense_count DESC`,
      [business_id, totalExpenses]
    );

    res.json({
      success: true,
      data: categoryRes.rows.map(r => ({
        category: r.category_name,
        count: parseInt(r.expense_count, 10),
...
exports.expenseAnalytics = async (req, res) => {
  try {
    ...
    let values = [business_id];
    let idx = 2;
    let whereClause = `e.business_id = $1`;

    if (branch_id) {
      whereClause += ` AND e.branch_id = $${idx}`;
    ...
    switch (period) {
      ...
      case 'custom':
        ...
        dateFilter = ` AND e.expense_date BETWEEN $${idx} AND $${idx + 1}`;
        values.push(start_date, end_date);
        idx += 2;
        break;
      default: break;
